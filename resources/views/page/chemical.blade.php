...
<script>
  ...
  function getData() {
    if(farmerCode){
        ...
        success: function(res) {
            currentDatas = res.data.chemicals || [];
            console.log(currentDatas);
            getTableByCode(currentDatas);
        },
        error: function() {
  ...
  function getTableByCode(data) {
    ...
    table = $('#chemicalsTable').DataTable({
      ...
      pageLength: 50,
      columns: [
        { data: 'chemical_code', name: 'รหัสสารเคมี' },
        { data: 'chemical_name', name: 'ชื่อสารเคมี' },
        { data: 'value', name: 'ชื่อสารเคมี' },
        { data: 'unit_name', name: 'ชื่อสารเคมี' },
        {
          data: null,
          orderable: false,
          searchable: false,
          render: function(data){
            return `
                <a class="btn btn-sm btn-info" href="/chemicals/${data?.chemical_id}">เบิกสารเคมี</a>`;
          }
        }
      ]
    });
